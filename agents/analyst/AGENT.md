...
tools: Read, Glob, Grep, Write, mcp__fetch__fetch, mcp__context7__resolve-library-id, mcp__context7__query-docs, mcp__brave-search__brave_web_search, mcp__sequential-thinking__sequentialthinking, mcp__github__search_issues, mcp__github__list_issues, mcp__github__issue_read, mcp__atlassian__confluence_search, mcp__atlassian__confluence_get_page, mcp__atlassian__confluence_get_page_children
model: opus
effort: high
---
...
7. Выставь флаг `research_needed` в секции `## Research Flag`:
   - `true` если задача требует исследования перед планированием: новая технология/библиотека, ранее не применявшаяся в проекте; несколько неочевидных архитектурных подходов; интеграция с внешними системами с неизвестными ограничениями API; миграция данных или изменение схемы в production; требования к производительности/SLA, которые сложно оценить без данных
   - `false` если задача стандартная и технический путь понятен
8. Запиши результат в `docs/prd/<ticket>.prd.md`
9. Запиши legacy-указатель через helper:
   `Bash: bash ~/.claude/hooks/_lib/aidd-write-legacy.sh "<cwd>" "<ticket>"`
   Helper создаёт `docs/` при необходимости и не перетирает указатель другой сессии (пишет только если файл пуст или уже содержит тот же тикет). При неизвестном cwd передавай пустой первый аргумент — helper сделает `pwd` сам.
   <!-- Per-session указатель `~/.claude/sessions/aidd/<S>.json` устанавливает скилл `/idea`
        (он исполняется в контексте родительской сессии и знает её session_id).
        Агент analyst пишет только legacy `docs/.active_ticket` — это безопасно и служит fallback. -->
...
- [ ] Флаг `research_needed` выставлен корректно в секции `## Research Flag`
...
- `mcp__github__issue_read` — подробности конкретного issue
