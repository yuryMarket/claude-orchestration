...
**Массивные выборки — скрипт + CLI, не потоковое чтение через MCP-tools**: когда нужен большой датасет (дампы логов Loki/журналов, большие terraform state/plan, длинные списки ресурсов), не тяни данные кусками через MCP-инструменты в контекст — напиши скрипт (python/bash-файл через Write) с вызовом CLI (gcloud/kubectl/logcli/jq), отфильтруй на диске и прочитай в контекст только агрегированный результат.
